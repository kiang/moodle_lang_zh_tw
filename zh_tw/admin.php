...
<p>運行此腳本，請訪問<a href="{$a->fixurl}">博客等級升級頁面</a>。</p>
';
$string['bloglevelupgradeinfo'] = '博客的可見性在2.0中簡化了，但您的網站仍在使用某種舊的可見性類型。為了保留基於課程或基於小組的博客可見性，下面的腳本會在有用戶發布博客的課程裡建立一個特殊的“博客”類型的討論區，並把這些博客條目拷貝到這個特殊討論區中。然後，所有博客在網站層面將被完全關閉。此過程中沒有博客條目會被刪除。';
$string['bloglevelupgradenotice'] = '您的網站正在使用舊的博客可見性設置，建議<a href="bloglevelupgrade.php">升級博客可見性</a>。';
$string['bloglevelupgradeprogress'] = '轉換進度：審核了{$a->userscount}個用戶，轉換了{$a->blogcount}篇博文。';
...
$string['configdebugstringids'] = '此選項是為方便翻譯字符串而設。它在輸出的字符串旁邊顯示對應的語言文件和字符串id。（修改此設置只會影響下一次頁面加載。）';
...
$string['configdefaultallowedmodules'] = '對於上面所選類別中的新建課程，在<b>剛剛創建時</b>允許它們使用哪些模塊?';
...
$string['configexcludeoldflashclients'] = '某些版本的Adobe Flash插件有可被惡意Flash攻擊的漏洞。您可以在這裡指定支持的最低版本。Moodle不會向使用更低版本的Flash的用戶顯示Flash文件。他們會看到一個講述如何升級的flash。此處留空會禁用所有的檢查。';
...
$string['configextrauserselectorfields'] = '在搜索用戶時（比如分配角色或向小組添加用戶），除了全名外，還有哪些字段也被搜索和顯示。安全起見，建議<em>不</em>要選擇用戶名字段。';
...
$string['configintrotimezones'] = '這個頁面用於搜索新的關於全世界時區的信息(包括夏時制規則)並更新您的本地數據庫。這些位置將會被按此次序逐一檢查：{$a}。這個過程通常是非常安全的，他不會破壞正常的安裝。您要更新您的時區嗎？';
...
$string['configrestrictbydefault'] = '上面所選的類別中新創建的課程在缺省情況下其模塊是否受限?';
$string['configrestrictmodulesfor'] = '哪些課程會受到禁用某些活動模塊<b>設置</b>的影響？注意，此設置僅對教師有效。管理員總是可以向課程添加任意活動。';
...
$string['confirminstall'] = '您准備安裝語言包({$a})，您確定嗎？';
...
$string['enablecourseajax_desc'] = '編輯課程主頁時可以使用AJAX。注意，課程格式和主題風格必須支持AJAX編輯，並且用戶必須在個人資料中啟用AJAX。';
...
$string['ignore'] = '忽略';
$string['importlangreminder'] = '數據庫遷移過程已經准備好了。您需要<b>重新登錄</b>以使其剩下。在數據庫遷移過程之後，請立刻通過 Admin->Lang->語言導入工具 安裝Unicode語言包。';
...
$string['iplookupnetgeonote'] = 'NetGeo服務器當前用來查詢您地理信息，要獲得更多准確的結果，強烈推薦安裝一個本地版的MaxMind GeoLite數據庫。';
$string['keeptagnamecase'] = '保留標示名稱的大小寫';
$string['lang'] = '預設語言';
$string['lang16notify'] = 'Moodle 1.6及其後續版本允許您從 download.moodle.org 直接安裝並更新語言包，點擊下面的連接就可以了。';
...
$string['langimportdisabled'] = '語言導入功能已被禁止。您必須手工在文件系統上更新語言包。';
...
$string['langrmyourself'] = '為了防止數據丟失，lang.php不能被覆蓋已經存在內容為空的文件。請手動刪除它。';
...
$string['multilangupgradeinfo'] = '多語言語法過濾在Moodle1.8中已經改變， <lang> 標簽已經不再支持<br /><br />例如: <span lang="en" class="multilang">Hello!</span><span lang="es" class="multilang">Hola!</span><br /><br /><strong>您想更新所有文本中的語法特性嗎？</strong>';
$string['multilangupgradenotice'] = '您的站點可能使用了舊的多語言語法特征，推薦使用<a href="multilangupgrade.php">多語言更新</a> 。';
...
$string['remotelangnotavailable'] = '由於 Moodle 無法連接到 download.moodle.org，我們無法自動完成語言包的安裝。請從 http://dopwnload.moodle.org 下載相應的 zip 文件，並將它們復制到 {$a} 目錄中解壓。';
...
$string['requiredtemplate'] = '必須的。您可能使用這裡的語法(%l = lastname, %f = firstname, %u = username)。更多信息查看幫助和舉例。';
...
$string['unicodeupgradenotice'] = '在 Moodle 1.6 中，我們已經將所有的語言遷移到 Unicode 編碼。要完成這個站點的升級您需要使用遷移腳本將您的數據庫中的數據全部轉換為 Unicode (UTF-8)。<a href="utfdbmigrate.php">點擊這裡以啟動遷移腳本</a>！';
$string['uninstall'] = '卸載選擇的語言包';
$string['uninstallconfirm'] = '您准備要完全卸載語言包{$a}，您確定嗎？';
...
$string['uploadpicture_baduserfield'] = '用戶指定屬性無效，請重試。';
$string['uploadpicture_cannotmovezip'] = '不能移動zip文件到臨時目錄。';
$string['uploadpicture_cannotprocessdir'] = '不能解壓文件。';
$string['uploadpicture_cannotsave'] = '不能保存用戶{$a}的圖片，片，檢查原圖片文件。';
$string['uploadpicture_cannotunzip'] = '不能解壓圖片文件。';
$string['uploadpicture_invalidfilename'] = '圖片文件名含有非法字符，跳過。';
$string['uploadpicture_overwrite'] = '覆蓋已有的用戶頭像？';
$string['uploadpictures'] = '上傳頭像';
$string['uploadpictures_help'] = '用戶頭像可以通過zip文件或圖片文件上傳。圖像文件應按chosen-user-attribute.extension的形式命名。例如，用戶user1234的頭像文件名應為user1234.jpg。';
$string['uploadpicture_userfield'] = '用來匹配圖片的用戶屬性：';
$string['uploadpicture_usernotfound'] = '用戶字段\'{$a->userfield}\'，值 \'{$a->uservalue}\' 不存在，跳過。';
$string['uploadpicture_userskipped'] = '跳過用戶{$a}(已經有一張圖片)';
