...
$string['auth_ldap_auth_user_create_key'] = '透過外部建立使用者';
...
$string['auth_ldap_create_context'] = '如果您允許根據email資訊建立使用者，請指定建立使用者的條件。該值基於安全問題應該有別於別的使用者者。您不需將此條件加到LDAP條件變數，Moodle以此條件搜尋使用者。<br /><b>注意!</b> 您必須變更auth/ldap/auth.php檔案中的user_create()方法以使建立使用者的工作能運作。';
$string['auth_ldap_create_context_key'] = '新使用者的條件';
...
$string['auth_ldapdescription'] = '該方法利用一個外部的LDAP伺服器進行身份驗證。 如果使用者名和密碼是有效的，本系統據此在資料庫中建立一個新使用者。 該模組可以從LDAP中讀取使用者屬性，並把指定的欄位預先填入本系統資料庫。 此後的登錄只需檢驗使用者名和密碼。';
...
$string['auth_ldap_search_sub'] = '從次級上下文中搜索使用者';
$string['auth_ldap_search_sub_key'] = '搜尋子條件';
$string['auth_ldap_server_settings'] = 'LDAP 伺服器設定';
$string['auth_ldap_unsupportedusertype'] = '認證： ldap user_create() 不支援選擇的使用者類型："{$a}" (..還沒)';
$string['auth_ldap_update_userinfo'] = '從LDAP向本系統更新使用者資訊（姓名、位址……）要查看對應資訊，請看/auth/ldap/attr_mappings.php';
$string['auth_ldap_user_attribute'] = '用於命名/搜索使用者的屬性。通常為『cn』。';
$string['auth_ldap_user_attribute_key'] = '使用者屬性';
...
$string['auth_ldap_user_type_key'] = '使用者類型';
