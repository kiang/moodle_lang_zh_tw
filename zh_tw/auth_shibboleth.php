...
$string['auth_shib_auth_method_description'] = '給Shibboleth驗證方法起一個使用者熟悉的名字。可以是您的Shibboleth組織的名字，比如<tt>SWITCHaai 登陸</tt> 或者 <tt>InCommon 登陸</tt> 或者其他類似的名字。';
$string['auth_shibboleth_contact_administrator'] = '若您無法與該組織連繫，而您需要存取此網站的課程，請與管理者連繫。';
$string['auth_shibbolethdescription'] = '這個方法，使用者的建立和驗證是使用<a href="http://shibboleth.internet2.edu/" target="_blank">Shibboleth</a>.<br>請閱讀 <a href="../auth/shibboleth/README.txt" target="_blank">README</a> 幫助您了解如何設定Moodle 來使用Shibboleth';
...
$string['auth_shib_idp_list_description'] = '提供身份提供者的 entityID 列表，供使用者在登錄頁面選擇。<br />每行都必須是一個半形逗號分隔的二元組，一個是 IdP（在Shibboleth 元數據文件中）的 entityID，另一個是顯示在下拉列表中的 IdP 的名字。<br />如果您的 Moodle 是多聯邦配置的一部分，那麼會使用可選的第三個參數，那是 Shibboleth 會話起始器的位置。';
$string['auth_shib_instructions'] = '如果您的學校支援Shibboleth，可以使用<a href="{$a}">Shibboleth登入</a>來存取它。<br />否則請使用這標準登入表單。';
$string['auth_shib_instructions_help'] = '應當在此為您的使用者提供關於Shibboleth的解釋。在登入畫面上，將會顯示這些提示。其中應包含一個指「<b>{$a}< /b>」的連結，這樣Shibboleth使用者可以輕鬆登入。如果您空白，預設的指示資訊將會被使用(不是Shibboleth特定的)。';
$string['auth_shib_integrated_wayf'] = 'Moodle WAYF 服務';
$string['auth_shib_integrated_wayf_description'] = '如果選中這一項，Moodle將會使用自己的WAYF服務代替原先為Shibboleth所配置的服務。Moodle會在這個新的登錄頁面上顯示一個下拉列表以供使用者選擇自己的身份提供者。';
$string['auth_shib_logout_return_url'] = '備用的登出返回URL';
$string['auth_shib_logout_return_url_description'] = '請提供Shibboleth使用者註銷後跳轉頁面的URL。<br/>使用者將會被重定向到Moodle預設的重定向地址。';
$string['auth_shib_logout_url'] = 'Shibboleth服務提供商退出處理URL';
$string['auth_shib_logout_url_description'] = '提供指向Shibboleth服務提供商退出處理的URL。一般形如<tt>/Shibboleth.sso/Logout</tt>';
...
$string['auth_shib_username_description'] = '用在Moodle中當作帳號稱的Shibboleth環境變數';
