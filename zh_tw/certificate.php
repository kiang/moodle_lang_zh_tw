...
$string['bordercolor_help'] = '由於圖像會大幅增加 pdf 文件的大小，您可以選擇列印邊界線，而不是使用邊界圖像（確保邊界圖像選項設置為否）。邊界線選項將列印一個使用選中顏色的不同寬度的三條線的漂亮邊界。';
...
$string['certificate:printteacher'] = '列印教師';
$string['certificatereport'] = '證書報告';
$string['certificatesfor'] = '證書';
$string['certificate:student'] = '獲取證書';
$string['certificatetype'] = '證書類型';
$string['certificatetype_help'] = '這是您確定證書布局的地方。證書類型文件夾包含了四種預設證書：
A4 嵌入式列印在 A4 大小的紙上，使用嵌入字體。
A4 非嵌入式列印在 A4 大小的紙上，不使用嵌入字體。
信件嵌入式列印在信件大小的紙上，使用嵌入字體。
信件非嵌入式列印在信件大小的紙上，不使用嵌入字體。

非嵌入類型使用 Helvetica 和 Times 字體。如果您擔心您的使用者的電腦上沒有這種字體，或者您的語言使用的字元或符號無法接受Helvetica 和 Times 字體，那麼選擇一種嵌入類型。嵌入類型使用 Dejavusans 和 Dejavuserif 字體。這會使得 PDF 文件變得相當大；因此不建議使用嵌入類型，除非您必須使用。

新類型文件夾可以新增到 certificate/type 文件夾里。文件夾的名字和新類型使用的任何語言字元串必須新增到證書的語言文件里。';
...
$string['customtext_help'] = '如果您想讓證書列印除已經分配為教師以外的不同教師的名字，不要選擇列印教師或除線條圖像以外的任何簽名圖像。在文字框里以您想要它們出現的形式輸入教師的名字。預設情況下，這個文字被放置在證書的左下角。以下 HTML 標籤可用： <br>，<p>，<b>，<i>，<u>，<img> （src 和 width（或 height）是強制性的），<a>（href 是強制性的），<font> （可能的屬性是：color，（hex color code）, face， (arial，times，courier，helvetica，symbol））。';
$string['date'] = '在';
$string['datefmt'] = '日期格式';
$string['datefmt_help'] = '選擇在證書上列印的日期的格式。或者選擇最後一項來按戶選擇的語言的日期格式來列印日期。';
$string['datehelp'] = '日期';
$string['deletissuedcertificates'] = '刪除頒發的證書';
$string['delivery'] = '交貨';
$string['delivery_help'] = '在這兒選擇您希望您的學生怎麼獲得他們的證書。
在瀏覽器中打開：在瀏覽器新視窗中打開證書。
強制下載：打開瀏覽器下載視窗。
電子郵寄證書：選擇此項來將證書作為電子郵件附件發送給學生。
在使用者收到證書後，如果他們在課程主頁上點擊證書連結，他們將看到他們獲得證書的日期並可以查看他們獲得的證書。';
...
$string['gradefmt_help'] = '如果您選擇在證書上列印成績，有三種可用的格式：
...
$string['opendownload'] = '點擊下面的按鈕把您的證書保存到您的計算機里。';
...
$string['printdate_help'] = '如果選擇了列印日期，這個日期將會列印出來。如果選擇了課程結業日期但學生沒有完成課程，將會列印獲得證書的日期。您也可以選擇列印活動評分的日期。如果證書在活動評分前頒發，將會列印獲得證書的日期。';
$string['printerfriendly'] = '列印友善頁面';
$string['printgrade'] = '列印成績';
$string['printgrade_help'] = '您可以從成績單中選擇任何可用的課程成績項，在證書上列印使用者在該項得到的成績。成績項以他們在成績單中的順序排列。在下面選擇成績格式。';
$string['printhours'] = '列印學分數';
$string['printhours_help'] = '輸入要列印在證書上的學分數';
$string['printnumber'] = '列印證書字號';
$string['printnumber_help'] = '證書上會列印一個隨機的字母和數字組成的獨一無二的 10 位號碼。通過和證書報告里顯示的號碼比較，可以驗證號碼。';
$string['printoutcome'] = '列印核心能力';
$string['printoutcome_help'] = '您可以將任何課程能力名和使用者得到的能力列印在證書上。例如：作業能力：精通。';
$string['printseal'] = '印章或機構標誌圖像';
$string['printseal_help'] = '這個選項允許您從 certificate/pix/seals 文件夾選擇一個印章或標誌列印在證書上。預設情況下，這個圖片被放在證書的右下角。';
$string['printsignature'] = '簽名圖像';
$string['printsignature_help'] = '這個選項允許您從  certificate/pix/signatures 文件夾列印一個簽名圖片。您可以列印一個圖形表示的簽名或列印一個用來書寫簽名的直線。預設情況下，這個圖片被放在證書的左下角。';
$string['printteacher'] = '列印教師姓名';
$string['printteacher_help'] = '為在證書上列印教師的名字， 設置教師的角色在模組水平上。舉例來說，如果您課程里有不止一個教師，或者您課程里有不止一個證書，並且您想要在每一個證書上列印不同的教師名字，就要這樣做。點擊編輯證書，然後點擊「本地分配的角色」 選項卡。然後分配角色教師（編輯教師）給證書（他們不一定是此課程中的教師 —— 您可以將角色分配給任何人）。這些名字將會作為教師列印在證書上。';
$string['printwmark'] = '浮水印圖像';
$string['printwmark_help'] = '一個水印文件將會放在證書的背景里。水印是一個淺色的圖形。水印可以是一個標誌、印章、飾章、文字或您想用來作為圖形背景的任何東西。';
