...
$string['dbsetupsql_desc'] = '設置某些數據庫的SQL命令。通常用來設置通信編碼 — 例如在MySQL和PostgreSQL中：<em>SET NAMES \'utf8\'</e一';
$string['dbsybasequoting'] = '使用sybase引號';
$string['dbsybasequoting_desc'] = 'Sybase風格的單引號轉義 — Oracle、MS SQL和其它某些數據庫需要。不要在MySQL上使用�一';
$string['dbtype'] = '資料庫型式';
$string['dbtype_desc'] = 'ADOdb數據庫驅動名，也就是外部數據塊引擎的類型�一';
$string['dbuser'] = '使用者';
$string['debugdb'] = '調試ADO一';
$string['debugdb_desc'] = '調試ADOdb與外部數據庫的連接 — 如登錄時顯示空白頁，就使用此選項。不適用於正式使用的網站�一';
$string['defaultcategory'] = '缺省的新課程類別';
$string['defaultcategory_desc'] = '自動創建的課程的缺省類別。當未指定類別id或類別id不存在時使用�一';
$string['defaultrole'] = '缺省角色';
$string['defaultrole_desc'] = '如果外部表中未指定其它角色，會缺省分配此角色�一';
$string['ignorehiddencourses'] = '忽略隱藏的課程';
$string['ignorehiddencourses_desc'] = '如果啟用，用戶不會被加入設為學生不可學習的課程�一';
...
$string['newcoursetable_desc'] = '指定一個表名，它應該包含所有要自動新建的課程。留空表示不創建任何課程�一';
$string['pluginname'] = '外部數據庫';
$string['pluginname_desc'] = '您可以使用幾乎所有類型的外部數據庫控制您的選課。您的外部數據庫至少要有一個課程ID字段和一個用戶ID字段。它們會和本地課程表和用戶表中您選擇的字段相匹配�一';
$string['remotecoursefield'] = '我們用來比對課程資料表的遠端資料表名稱欄位';
$string['remotecoursefield_desc'] = '用來和課程表匹配的遠程表中的字段名�一';
$string['remoteenroltable'] = '遠程用戶選課數據表';
$string['remoteenroltable_desc'] = '指定包含用戶選課信息的表名。留空表示不同步任何用戶的選課�一';
...
$string['templatecourse_desc'] = '可選：自動創建的課程可以從模板課程拷貝設置。在此輸入模板課程的簡稱�一';
