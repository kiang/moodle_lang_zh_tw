...
如果源文件是個**單位文件**，比如說Hot Potatoes masher （熱土豆碾子）文件或者index.html，該單位文件所列出的所有測驗會被以相同的設置添加進來，並形成一個測驗鏈。';
...
$string['configmaxeventlength'] = '如果一個HotPot既指定了開啟也指定了關閉時間，且兩個時間的差大於此處指定的天數，則將會添加兩個單獨的日歷事件到課程日歷中。對於時間差小於指定日期數，或者只指定了一個時間的情況，只添加一個日歷事件。如果兩個時間均未被指定，將不會添加日歷事件。';
$string['configstoredetails'] = '若啟用該設置，則HotPot測驗中的原始XML細節將會存儲在hotpot_details表中。這允許提交的測驗答案可以在以後重新評分，以反映HotPot測驗評分系統中的變化。然而，如果在一個繁忙的站點上啟用該選項，hotpot_details表中的數據會增長得非常快。';
$string['confirmdeleteattempts'] = '您真的要刪除這些提交麼？';
...
**答卷**
：如果選中，一個顯示用戶以前在此 HotPot 的答卷詳細信息的表格將會顯示在入口頁面上。答卷如果中斷，在最右邊的欄裡將會顯示一個繼續按鈕。';
...
$string['exit_attemptscore'] = '您的那次答卷成績是 {$a}';
$string['exitcm'] = '下一項活動';
...
如果其他退出頁面選項被禁用了，學生將會直接進入下一項活動。否則將會給學生顯示一個鏈接，當他們准備好時可以帶他們進入下一項活動。';
...
另外，如果單元評分方式是最高的，將會顯示一條消息，告訴用戶最新的答卷是等於還是優於前次。';
...
$string['forceplugins_help'] = '如果啟用，兼容 Moodle 的媒體播放器將會播放 avi、mpeg、mpg、mp3、mov 和 wmv 等文件。否則，Moodle 不會修改測驗裡任何媒體播放器的設置。';
...
$string['navigation_moodle'] = '標准 Moodle 導航欄（頂部和側邊）';
...
$string['sourcefilenotfound'] = '源文件丟失（或為空）：{$a}';
$string['status'] = '狀態';
...
$string['usefilters_help'] = '如果此設置開啟，內容在被發送到用戶的瀏覽器以前將會通過 Moodle 的過濾器。';
$string['useglossary'] = '使用詞匯表';
$string['useglossary_help'] = '如果此設置啟用，內容在被發送到瀏覽器前會通過 Moodle\'s Glossary Auto-linking 過濾器。

注意此設置會覆蓋網站管理設置來開啟或禁用 Moodle\'s Glossary Auto-linking 過濾器。';
