...
$string['nativemssql'] = 'SQL*伺服器 FreeTDS (native/mssql)';
$string['nativemssqlhelp'] = '現在，您需要配置資料庫，Moodle的大部分數據都將保存於此。
這個資料庫必須已經建立，並且有帳號和密碼可以訪問它。必須設置表前綴。';
$string['nativemysqli'] = '改進的MySQL (native/mysqli)';
$string['nativemysqlihelp'] = '現在，您需要配置資料庫，Moodle的大部分數據都將保存於此。
帳號和密碼必須已經存在。如果該使用者有相應權限，資料庫會被自動建立。表前綴可選。';
$string['nativeoci'] = 'Oracle (native/oci)';
$string['nativeocihelp'] = '現在，您需要配置資料庫，Moodle的大部分數據都將保存於此。
這個資料庫必須已經建立，並且有帳號和密碼可以訪問它。必須設置表前綴。';
$string['nativepgsql'] = 'PostgreSQL (native/pgsql)';
$string['nativepgsqlhelp'] = '現在，您需要配置資料庫，Moodle的大部分數據都將保存於此。
這個資料庫必須已經建立，並且有帳號和密碼可以訪問它。必須設置表前綴。';
$string['nativesqlsrv'] = 'SQL*伺服器Microsoft (native/sqlsrv)';
$string['nativesqlsrvhelp'] = '現在，您需要配置資料庫，Moodle的大部分數據都將保存於此。
這個資料庫必須已經建立，並且有帳號和密碼可以訪問它。必須設置表前綴。';
...
$string['pathssubadmindir'] = '有些網路主機使用/admin這個URL來訪問控制面板或其它功能。很不幸，這個設置和Moodle管理頁面的標準路徑衝突。這個問題可以解決，只需在您的安裝目錄中把admin目錄換名，然後把新名字輸入到這裡。例如<em>moodleadmin</em>。這麼做會改變Moodle中的管理連結。';
$string['pathssubdataroot'] = 'Moodle需要一個位置存放上傳的文件。這個目錄對於Web伺服器使用者(通常是「nobody」或「apache」)應當是可讀可寫的，但應當不能直接通過Web訪問它。如果它不存在，安裝程序會嘗試建立。';
$string['pathssubdirroot'] = 'Moodle安裝的完整目錄徑路';
$string['pathssubwwwroot'] = '可以訪問到Moodle的完整網址。
Moodle不支持通過多個地址訪問。如果您的網站有多個公開地址，您必須把這個地址以外的所有地址都設為永久重定向。如果您的網站既可以通過內部地址訪問，也可以通過這個公開地址訪問，那麼請配置DNS使內部網使用者也能使用公開地址。如果此地址不正確，請在瀏覽器中修改URL來重新安裝，並設定另一個地址。';
