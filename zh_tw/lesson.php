...
$string['cannotfindpagerecord'] = '錯誤：找不到頁面紀錄(在新增分支結尾)';
...
$string['cannotfinduser'] = '錯誤：無法找到使用者';
...
　　
　　<p>開啟此選項後，會顯示此課中頁面(分支表)的列表。問題頁和簇頁預設情況下不在其中(可以修改問題頁屬性來顯示它)。</p>';
...
<p>這個功能於允許您通過表單上傳外部文字檔案，並匯入其中的問題。</p>
<p>它可以支援很多檔格式：</p>
<p><b>GIFT格式</b></p>
<ul>
<p>GIFT是Moodle可以用來從文字檔案匯入問題的最複雜的格式。它支援單選題、是非題、簡答題、配合題、
...
<p><b>AON</b></p>
<ul>
<p>這個格式在多數情況下和Missing Word格式相同，但在匯入後，所有簡單題都會被轉為辨識題。</p>
<p>此外，多個答案在匯入的過程中其順序會被打亂。</p>
<p>它以一個開發了多種測驗功能的組織命名。</p>
</ul>
　　
<p><b>Blackboard</b></p>
<ul>
<p>這個模組可以匯入以Blackboard的匯出格式保存的問題。要使用它，PHP必須具備XML處理函數。</p>
　　
...
<p><b>自行定義</b></p>
<ul>
<p>如果您希望以您自己的格式匯入問題，請在「mod/quiz/format/custom.php」檔中實現它。</p>

<p>需要編寫的新代碼並不多——只需從給定的文字中分析出試題就可以了。</p>
　　
<p align="right"><a href="help.php?file=formatcustom.html&module=quiz">"自訂"格式的更多資訊</a></p>
</ul>
　　
...
<p>對於已經有內容的單元，修改這個值並不會有問題。事實上，如果您想新增一個有很多選項的問題，或者是很長的分支表
...
　　
　　<p>此選項的預設值為5，太小的值會導致學生不仔細思考問題，太大的值會帶來比較大的挫折感。</p>
　　
　　<p>設定此選項的值為1，則只給學生一次回答的機會，這和測驗模組比較像，不同之處在於問題是每個頁面只有一個問題。</p>
　　
　　<p>注意這個選項是全域性質的，課中無論哪種類型的題目都會受其影響。</p>
　　
...
　　
　　<p>這個選項只用於卡片形式的課。預設值0表示此課中所有的頁面/卡片都會顯示出來，而設定此選項為非零值將會顯示相應數目的頁面。在顯示完指定頁面/卡片後，系統會結束此課並將成績顯示出來。</p>
　　
...
<li>每一個答案都有一小段文字與之關聯，當該答案被選擇時系統將會把這段文字顯示給使用者。這段文字被稱為答案的<b>回應</b>。</li>

<li>每一個答案還有一個<b>跳轉到</b>鏈結與之相連。跳轉到鏈結可以是相對的(如本頁、下一頁)或絕對的(課中的指定頁面或課程結束頁)。</li>

<li>預設的情況下，第一個答案會跳到該課的<b>下一頁</b>。後續的答案都跳轉到本頁，也就是說如果學生沒有選擇第一個答案，則系統還是將當前頁面顯示給他們。如果您已經建立了一個有簇，則可以選擇跳轉到簇中的未讀頁面，如果當前您並不在簇中，則看不到這個選項。您可以在任何時刻將一組問題組合成簇。</li>
...
給學生的答案都會不同。)有一個例外是辨識題，在這種題目中，答案的順序是和教師輸入的順序一致的。</li>
...
<p>這個選項的預設值為<b>是</b>，表示學生可以重學此課。實際上只有一些個別的情況這個值才應當設定為<b>否</b>。</p>
　　';
...
　　
　　<p>在這種情況下，課以幻燈片的形式來顯示。幻燈片有固定的寬度、高度和可定制的背景顏色。如果頁面的內容超過了指定的寬度和高度，則會顯示基於CSS的捲軸。問題將會「打斷」幻燈片的顯示，預設情況下只會顯示頁面(分支表)。如果選定了相應的選項，則在頁面的最左或最右將會將會以相應的語言顯示「向前」或「向後」按鈕，而其他的按鈕在幻燈片的中下部顯示。</p>
　　';
