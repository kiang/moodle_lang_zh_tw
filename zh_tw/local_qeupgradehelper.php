...
$string['croninstructions'] = '在部分升級之後，您可以讓cron自動完成剩下的升級任務。Cron會在設置的時間段（服務器的本地時間）內運行。每次cron運行時，都會處理處理幾條試答，直到時限用盡，然後再等到下一次cron時運行。即使您設置了cron，它也可能什麼都不做，除非它檢測到向2.1的主要升級已經完成。';
...
$string['extracttestcase_desc'] = '用數據庫中的例子數據創建測試用例，用來測試升級。';
...
$string['listtodointro'] = '這是所有有試答且需要轉換的測驗。您可以點擊鏈接轉換試答。';
...
$string['notupgradedsiterequired'] = '此腳本只能在網站升級前使用。';
...
$string['pretendupgrade_desc'] = '升級做三件事：從數據庫加載已存在的數據；轉換數據；將轉換過的數據寫入數據庫。此腳本測試此過程中的前兩步。';
...
$string['upgradedsitedetected'] = '看到此條信息，說明此站已經升級到了新的試題引擎。';
$string['upgradedsiterequired'] = '此腳本只能在已經升級過的網站使用。';
$string['upgradingquizattempts'] = '正升級課程{$a->shortname}的測驗“{$a->name}”中的試答';
$string['veryoldattemtps'] = '在從 Moodle 1.4 升級到 Moodle 1.5 的過程中，您的網站有 {$a} 次試卷沒有被正確更新。這些試卷會在主升級之前被處理。您要對額外消耗的時間有心理准備。';
