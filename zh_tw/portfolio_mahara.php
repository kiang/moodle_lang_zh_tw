...
$string['err_invalidhost_help'] = '此插件的配置錯誤地指向了一個無效（或已刪除）的 MNet 主機。此插件依賴發布了 SSO IDP、訂閱了 SSO_SP 和 訂閱<b>且</b>發布了雲存儲的 Moodle 網絡伙伴。';
...
$string['err_nomnethosts_help'] = '此插件要求 MNet 伙伴要發布 SSO IDP、訂閱 SSO SP、發布雲存儲服務<b>並且</b>也訂閱了MNet認證插件。在條件不都滿足之前，此插件的所有實例都會隱藏。需要手工設置它們為可見。';
$string['failedtojump'] = '與遠程服務器通信失敗';
$string['failedtoping'] = '與遠程服務器通信失敗：{$a}';
...
$string['pf_description'] = '允許用戶推送 Moodle 內容到此主機<br />訂閱<b>並</b>發布此服務可以讓此網站的認證用戶推送內容到{$a}<br /><ul><li><em>相關</em>：您必須也<strong>發布</strong> SSO（身份提供者）服務到{$a}。.</li><li><em>相關</em>：您必須也<strong>訂閱</strong>{$a}的 SSO（身份提供者）服務</li><li><em>相關</em>：您必須也啟用 MNet 認證插件。</li></ul><br />';
