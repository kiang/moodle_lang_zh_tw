...
請注意，此度量不會顯著提高服務器的安全性，不過它可能減慢或限制一般的攻擊。</p>';
...
<p>缺省用戶對應的舊角色只能是<em>已認證用戶</em>。一定不能有查看課程權限。</p>';
...
$string['check_displayerrors_details'] = '<p>不建議在正式網站打開PHP設置<code>display_errors</code>，因為錯誤信息可能會暴露服務器上的一些敏感信息。</p>';
$string['check_displayerrors_error'] = '顯示錯誤信息的PHP設置被打開。建議關閉它。';
...
<p>認證插件中可能已經鎖定了email域，這一步在這裡沒有考慮。</p>';
$string['check_emailchangeconfirmation_error'] = '用戶可以使用任意 Email 地址。';
$string['check_emailchangeconfirmation_info'] = '用戶僅能使用特定域名的 Email 地址。';
$string['check_emailchangeconfirmation_name'] = 'Email更改的確認';
$string['check_emailchangeconfirmation_ok'] = '修改用戶信息中的 Email 時需確認。';
$string['check_embed_details'] = '<p>毫無限制的對象嵌入是非常危險的——任何注冊用戶都可以發起針對其它用戶的XSS攻擊。此設置在正式網站上應該禁用。</p>';
$string['check_embed_error'] = '允許無限制地嵌入對象——對於大多數服務器而言，這非常危險。';
...
<p>強烈建議為此創建一個新的角色且不要給它賦予任何傳統角色。</p>';
...
$string['check_globals_error'] = '一定要禁止注冊全局變量。請立刻修改服務器的PHP設置！';
$string['check_globals_name'] = '注冊全局變量';
$string['check_globals_ok'] = '注冊全局變量已禁用。';
...
$string['check_mediafilterswf_details'] = '<p>自動嵌入swf非常危險——任何注冊用戶都可能發起針對其它用戶的XSS攻擊。請在正式服務器禁用它。</p>';
$string['check_mediafilterswf_error'] = 'Flash 媒體過濾器已經激活——對於大多數服務器而言，這是非常危險的。';
$string['check_mediafilterswf_name'] = '激活的 .swf 媒體過濾器';
$string['check_mediafilterswf_ok'] = 'Flash 媒體過濾器未激活。';
$string['check_noauth_details'] = '<p><em>不認證身份</em>插件不是給正式網站設計的。除非這是一個開發測試網站，否則請禁用它。</p>';
$string['check_noauth_error'] = '不需認證插件不可以用在正式網站';
$string['check_noauth_name'] = '不需認證';
$string['check_noauth_ok'] = '不需認證插件是被關閉的';
$string['check_openprofiles_details'] = '<p>開放用戶個人信息可能會被不良用戶濫用。建議啟用<code>強制用戶登錄後才能訪問個人資料</code>或者<code>強制用戶登錄</code>。</p>';
$string['check_openprofiles_error'] = '任何人無須登錄就可以查看用戶的個人信息。';
$string['check_openprofiles_name'] = '開放用戶個人信息';
$string['check_openprofiles_ok'] = '在查看用戶個人信息前需登錄。';
$string['check_passwordpolicy_details'] = '<p>建議您設定一個密碼策略，因為猜測密碼是最常見的非法入侵方法。同時您也不要把密碼策略設定的太苛刻，這會導致用戶無法記住他們的密碼以至於忘記密碼或把密碼寫下來。</p>';
$string['check_passwordpolicy_error'] = '密碼策略未設置。';
$string['check_passwordpolicy_name'] = '密碼策略';
$string['check_passwordpolicy_ok'] = '密碼策略已激活。';
...
<p>建議只在系統場景下分配管理員角色。下列用戶在其它場景中被分配了管理員角色（不支持）：</p>{$a->unsupported}';
$string['check_riskadmin_name'] = '管理員';
$string['check_riskadmin_ok'] = '找到了 {$a} 個服務器管理員。';
$string['check_riskadmin_unassign'] = '<a href="{$a->url}">{$a->fullname} （{$a->email}）評估角色分配</a>';
$string['check_riskadmin_warning'] = '找到 {$a->admincount} 個服務器管理員和 {$a->unsupcount} 個不支持的管理員角色分配。';
$string['check_riskbackup_detailsok'] = '沒有被明確允許備份用戶數據的角色。但是，請注意，有“doanything”權限的管理員仍然可以這麼做。';
$string['check_riskbackup_details_overriddenroles'] = '<p>這些有效的角色覆蓋使用戶獲得在備份中包含其它用戶數據的權限。請確認此權限是必須的。</p> {$a}';
$string['check_riskbackup_details_systemroles'] = '<p>下列系統角色有在備份中包含用戶數據的權限。請確認此權限是必須的。</p> {$a}';
$string['check_riskbackup_details_users'] = '<p>因為上面的角色或局部覆蓋，下列用戶現在可以在備份中包含選修他們課程的所有用戶的隱私數據。請確認他們是可信任的，且他們的密碼足夠強壯：</p> {$a}';
...
$string['check_riskbackup_warning'] = '找到 {$a->rolecount} 個角色、{$a->overridecount}個覆蓋和 {$a->usercount} 個用戶有備份用戶數據的權限。';
$string['check_riskxss_details'] = '<p>有 RISK_XSS 標記的權限是危險的權限，只能允許可信任的用戶使用。</p>
<p>請確認下面的用戶列表，確認此服務器完全信任他們：</p><p>{$a}</p>';
$string['check_riskxss_name'] = 'XSS 信任用戶';
$string['check_riskxss_warning'] = 'RISK_XSS——發現 {$a} 個必須被信任的用戶。';
